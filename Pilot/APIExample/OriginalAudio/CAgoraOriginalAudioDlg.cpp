...
void  COriginalAudioProcFrameObserver::AgoraNDISend1(AudioFrame& audioFrame)
{
	...
	//audioFrame.buffer = "000000000";


	int bsize = bfsize((unsigned char*)audioFrame.buffer);
	int fsize = bffsize((float*)audioFrame.buffer);

	std::copy((float*)audioFrame.buffer, (float*)audioFrame.buffer + bsize, NDI_audio_frame.p_data);

	char msg[256];
	sprintf_s(msg, 256, "\nSending NDI frames ( channels=%d ; samples=%d ; strides=%d ; bsize:%d, fsize:%d )...\n",
...
void  COriginalAudioProcFrameObserver::AgoraNDISend2(AudioFrame& audioFrame)
{
...
void  COriginalAudioProcFrameObserver::AgoraNDISend3(AudioFrame& audioFrame)
{
	// TEST -START
